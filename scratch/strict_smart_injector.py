...
def run():
    print("[Jarvis X]: Monitoring active window. Please switch to the actual Swayam NPTEL ASSIGNMENT tab now.")
    target_found = False
    for _ in range(120):
        title = get_active_window_title()
        if title:
            title_lower = title.lower()
            # Must contain swayam or nptel, AND must not be a google search
            if ('swayam' in title_lower or 'nptel' in title_lower) and 'google search' not in title_lower:
