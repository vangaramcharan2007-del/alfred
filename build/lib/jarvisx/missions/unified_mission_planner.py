...
class UnifiedMissionPlanner:
    ...
    async def generate_plan_async(self, goal: str, memory_context: str = "") -> MissionPlan:
        ...
        resp = await self.router.route_request(prompt, require_offline=False)
        raw_text = ""
    ...
    async def execute_mission_async(
        ...
        """Asynchronously execute unified mission end-to-end with result propagation and synthesis."""
        ...
        # 2. Generate and Validate Plan asynchronously
        plan = await self.generate_plan_async(goal, memory_context=memory_ctx)
